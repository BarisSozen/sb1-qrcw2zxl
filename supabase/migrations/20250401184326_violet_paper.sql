...
-- Create policies
CREATE POLICY "Enable full access for authenticated users" ON sales_commission_rates
  FOR ALL TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "Enable full access for authenticated users" ON sales_commission_payments
  FOR ALL TO authenticated
  USING (true)
  WITH CHECK (true);

-- Create indexes
CREATE INDEX IF NOT EXISTS idx_commission_rates_sales_rep ON sales_commission_rates(sales_rep_id);
CREATE INDEX IF NOT EXISTS idx_commission_rates_client ON sales_commission_rates(client_id);
CREATE INDEX IF NOT EXISTS idx_commission_payments_sales_rep ON sales_commission_payments(sales_rep_id);
CREATE INDEX IF NOT EXISTS idx_commission_payments_status ON sales_commission_payments(status);
